...
void Game :: initMonsters ()
{
	...
	for (int r = 0; r < ROWS; r++)
		for (int c = 0; c < COLUMNS; c++)
		{
			...
			if (levels[current_level].getValue(pos) == SENTRY_START)
			{
				monsters[next_monster] = new Sentry(pos);
				next_monster++;
			}
            else if (levels[current_level].getValue(pos) == DRONE_START){
                monsters[next_monster] = new Drone(pos);
                next_monster++;
            }
            else if (levels[current_level].getValue(pos) == ATTACKER_START){
